...
contract Professional_v7 is Ownable {
    ...
    function withdraw() public _staked(msg.sender) returns (bool) {
        address from = msg.sender;
        require(
            block.timestamp >= deposits[from].endTime, //(deposits[from].depositTime).add(60), //(lockduration * 24 * 3600),
        ...
        returns (bool)
    {
        ...
        deposits[staker] = Deposits(
            ...
            block.timestamp.add(lockduration), //lockduration.mul(86400)
            index,
            false,
            stakerEligibility
        );
