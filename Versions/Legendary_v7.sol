...
contract Legendary_v7 is Ownable {
    ...
    IRookieProfessionalCheck public RookieInterface;
    IRookieProfessionalCheck public ProfessionalInterface;
    ...
    constructor(
        ...
        require(rookieAddress_ != address(0), "Rookie address: 0 address");
        rookieAddress = rookieAddress_;
        require(
            professionalAddress_ != address(0),
            "Rookie address: 0 address"
        );
    ...
    function withdraw() public _staked(msg.sender) returns (bool) {
        ...
        uint256 _endTime = deposits[from].endTime;
        return _withdrawAfterClose(from, amount, _userIndex, _depositTime, _endTime);
    }
    ...
    function _withdrawAfterClose(
        ...
        uint256 payOut = _calculate(amount, userIndex, depositTime, endTime);
    ...
    // function Check(address from) public returns (bool) {
    //     RookieInterface = IRookieProfessionalCheck(rookieAddress);
    //     ProfessionalInterface = IRookieProfessionalCheck(professionalAddress);
    //     (uint256 a, , , ) = RookieInterface.userDeposits(from);
    //     if (a <= RookieInterface.viewEligibilityAmount()) {
    //         RookieInterface.changeEligibility(from);
    //     }
    //     (uint256 b, , , ) = ProfessionalInterface.userDeposits(from);
    //     if (b <= ProfessionalInterface.viewEligibilityAmount()) {
    //         ProfessionalInterface.changeEligibility(from);
    //     }
    //     return true;
    // }
    ...
    function calculate(
        ...
        return _calculate(amount, userIndex, depositTime, endTime);
    }
    ...
    function _calculate(
        ...
        for (uint256 i = userIndex; i < index; i++) {
            ...
            if (endTime < rates[i + 1].timeStamp) {
                ...
                uint256 denom = _lockduration.mul(10000);
        ...
        if (depositTime < endTime) {
...

            {
                ...
                uint256 denom = _lockduration.mul(10000); //Replace with (lockduration * 24 * 3600 * 10000)
    ...
    //             ); //replace with (_lockduration * 24 * 3600 * 10000)
    //             amount += interest;
    //             depositTime = rates[i + 1].timeStamp;
    //             userIndex++;
    //         }
    ...
    //             .div(_lockduration.mul(10000)); //replace with (lockduration * 24 * 3600 * 10000)
    ...
    modifier _isEligible(address from) {
        RookieInterface = IRookieProfessionalCheck(rookieAddress);
        ProfessionalInterface = IRookieProfessionalCheck(professionalAddress);
        bool isRookieEligible = RookieInterface.eligibility(from);
        bool isProfessionalEligible = ProfessionalInterface.eligibility(from);
        require(
            isRookieEligible || isProfessionalEligible,
            "Stake at least 5k tokens in the Rookie pool pr Professional"
        );
        _;
    }

    // modifier _minStake(uint256 amount_) {
    //     require(amount_ >= minStakeAmount, "Please stake amount more than min Stake amount");
    //     _;
    // }
